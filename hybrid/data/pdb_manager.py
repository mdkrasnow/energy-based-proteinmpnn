...
class PDBManager:
    ...
    def _build_structure_index(self) -> None:
        ...
        self.logger.info(f"Built structure index with {len(self.structure_index)} structures")
    ...
    def _matches_filters(self, metadata: Optional[PDBMetadata], filters: Dict[str, Any]) -> bool:
    ...
    def validate_structure(self, pdb_id: str) -> Dict[str, Any]:
        ...
        metadata = self.get_metadata(pdb_id)
        if not metadata:
            return {"status": "missing_metadata", "issues": ["No metadata available"], "quality_score": 0.0}
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "metadata_db_path": str(self.metadata_db_path) if self.metadata_db_path else None,
        }
        
        # Resolution statistics
        resolutions = [m.resolution for m in self.metadata_db.values() if m.resolution is not None]
        if resolutions:
            stats.update({
    ...
    def refresh_index(self) -> None:
        ...
        self.logger.info(f"Structure index refreshed: {old_count} -> {new_count} structures")
...
class PDBListManager:
    ...
    def get_filtered_pdb_list(
        ...
        if use_cache:
            ...
            if cached_list and len(cached_list) >= target_count:
                self.logger.info(f"Using cached PDB list with {len(cached_list)} structures")
        ...
        try:
            ...
            if pdb_list:
                self.logger.info(f"Successfully retrieved {len(pdb_list)} PDB structures from RCSB")
                # Cache the results
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt > 0:
                    ...
                    self.logger.info(f"Retrying RCSB query in {delay:.2f}s (attempt {attempt + 1})")
                    time.sleep(delay)
                ...
                if response.status_code == 200:
                    try:
                        ...
                        if not isinstance(data, dict):
                            self.logger.error(f"API returned non-dict JSON: {type(data)}")
                            continue
                    ...
                    self.logger.warning(f"RCSB API error: {response.status_code} - {response.text}")
    ...
    def _validate_api_response(self, response_data: Any) -> bool:
        ...
        if not (has_result_set or has_results):
            self.logger.error("API response missing both 'result_set' and 'results' fields")
            return False
            ...
            if has_result_set:
                ...
                for idx, entry in enumerate(result_list[:5]):  # Check first 5 entries
                    if not isinstance(entry, dict):
                        self.logger.error(f"result_set[{idx}] must be dict, got {type(entry)}")
                        return False
                    if "identifier" not in entry:
                        self.logger.error(f"result_set[{idx}] missing 'identifier' field")
                        return False
                    if not isinstance(entry["identifier"], str):
                        self.logger.error(f"result_set[{idx}] identifier must be string, got {type(entry['identifier'])}")
                        return False
                ...
                for idx, entry in enumerate(result_list[:5]):  # Check first 5 entries
                    if not isinstance(entry, dict):
                        self.logger.error(f"results[{idx}] must be dict, got {type(entry)}")
                        return False
                    if "identifier" not in entry:
                        self.logger.error(f"results[{idx}] missing 'identifier' field")
                        return False
                    if not isinstance(entry["identifier"], str):
                        self.logger.error(f"results[{idx}] identifier must be string, got {type(entry['identifier'])}")
                        return False
                ...
                for entry in response_data["result_set"]:
                    if isinstance(entry, dict) and "identifier" in entry:
                        ...
                        if isinstance(identifier, str):
                            ...
                            self.logger.warning(f"Non-string identifier in result_set: {identifier}")
                ...
                for entry in response_data["results"]:
                    if isinstance(entry, dict) and "identifier" in entry:
                        ...
                        if isinstance(identifier, str):
                            ...
                            self.logger.warning(f"Non-string identifier in results: {identifier}")
            
            self.logger.debug(f"Extracted {len(pdb_ids)} PDB IDs from validated response")
            return pdb_ids
    ...
    def _validate_pdb_ids(self, pdb_ids: List[str]) -> List[str]:
        ...
        for pdb_id in pdb_ids:
            if not isinstance(pdb_id, str):
                self.logger.warning(f"PDB ID must be string, got {type(pdb_id)}: {pdb_id}")
                continue
            ...
            if len(pdb_id_clean) != 4:
                self.logger.warning(f"Invalid PDB ID length {len(pdb_id_clean)}, must be exactly 4: {pdb_id}")
                continue
    ...
    def _save_cached_list(self, pdb_ids: List[str]) -> None:
        ...
        try:
            ...
            try:
                ...
                self.logger.info(f"Atomically cached {len(pdb_ids)} PDB IDs to {self.filtered_list_cache}")
    ...
    def _get_fallback_list(self) -> List[str]:
        ...
        base_pdbs = [
            ...
            "3HTN", "4YOW", "4GYT", "6EHB", "5L33", "6MRR",
            # Additional high-quality protein structures
            "1UBQ", "1VII", "2CRO", "1ROP", "1TEN", "1A1U", "1BPI", "1C8C",
            "1D3Z", "1E0G", "1F39", "1G6X", "1H8H", "1I6H", "1J8H", "1K8A",
            "1L2Y", "1M1K", "1N8R", "1O91", "1P9I", "1Q1O", "1R7J", "1S3R",
            "1T1K", "1U06", "1V70", "1W0N", "1X6Z", "1Y8A", "1Z21", "2A3D",
            "2B97", "2C71", "2D57", "2E3H", "2F1K", "2G36", "2H5C", "2I4D",
            "2J28", "2K39", "2L0J", "2M1X", "2N3G", "2O8V", "2P5K", "2Q52",
            "2R11", "2S3R", "2T4B", "2U89", "2V7Q", "2W2T", "2X23", "2Y2V",
            "2Z5X", "3A04", "3B2Q", "3C8Z", "3D9A", "3E23", "3F1P", "3G21",
            "3H42", "3I37", "3J81", "3K0N", "3L1P", "3M1I", "3N44", "3O2S",
            "3P32", "3Q15", "3R9K", "3S8G", "3T1Y", "3U17", "3V6O", "3W2T",
            "3X42", "3Y8A", "3Z86", "4A1A", "4B2L", "4C4R", "4D43", "4E46",
            "4F3T", "4G36", "4H03", "4I55", "4J52", "4K81", "4L57", "4M1K",
            "4N12", "4O29", "4P4S", "4Q21", "4R17", "4S1I", "4T4F", "4U1S",
            "4V24", "4W53", "4X22", "4Y4O", "4Z4I", "5A63", "5B31", "5C8X",
            "5D2Q", "5E61", "5F73", "5G28", "5H57", "5I9R", "5JDE", "5KPY",
            "5LCX", "5M7E", "5N2D", "5O31", "5P21", "5Q4K", "5R1D", "5S3A",
            "5T4V", "5U4I", "5V74", "5W2G", "5X0D", "5Y1N", "5Z23", "6A5J"
        ]
        ...
        self.logger.warning(f"Using fallback PDB list with {len(replicated_list)} entries")
...
class StructureValidator:
    ...
    def validate(self, structure_data: Any) -> Dict[str, Any]:
        ...
        if not structure_data:
            return {"status": "invalid", "issues": ["No structure data provided"], "quality_score": 0.0}
        ...
        if quality_score < min_quality:
            issues.append(f"Quality score {quality_score:.2f} below minimum {min_quality}")
